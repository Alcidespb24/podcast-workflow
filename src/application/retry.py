...
def retry_with_backoff(
    ...
    * Backoff formula: ``initial * multiplier ** (attempt - 1)``, capped at *backoff_max*.
    * If *shutdown_event* is set during a wait, ``InterruptedError`` is raised.
    """
    retries = 0
    attempt = 0  # for backoff exponent (includes rate-limit waits)
    ...
    while True:
        try:
            ...
            delay = exc.retry_after if exc.retry_after is not None else _backoff(
                attempt, backoff_initial, backoff_multiplier, backoff_max,
            )
            ...
            if retries > max_retries:
                raise
            delay = _backoff(attempt, backoff_initial, backoff_multiplier, backoff_max)
            logger.warning(
